...
fn bench_pos1(c: &mut Criterion) {
    let grad = build_gradient(BlendMode::Rgb);
    c.bench_function("CatmullRomGradient position 0.01", |b| {
        b.iter(|| {
...
fn bench_pos2(c: &mut Criterion) {
    let grad = build_gradient(BlendMode::Rgb);
    c.bench_function("CatmullRomGradient position 0.5", |b| {
        b.iter(|| {
...
fn bench_pos3(c: &mut Criterion) {
    let grad = build_gradient(BlendMode::Rgb);
    c.bench_function("CatmullRomGradient position 0.99", |b| {
        b.iter(|| {
